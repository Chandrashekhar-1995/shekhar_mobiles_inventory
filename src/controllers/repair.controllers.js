...
const createRepair = asyncHandler(async (req, res, next) => {
    try {
        ...
        if(bookedByUser){ //check if bookedByUser exists
...
const fetchTodayRepairBookingSummary = asyncHandler(async (req, res, next) => {
    try {
        const [result] = await Repair.getTodayRepairBookingSummary();           
        const todaySummary = result ? result : { totalRepairPrice: 0, bookRepairCount: 0 };
    ...
    try {
        ...
        const repairs = await Repair.find({
            $or: [
                ...
                { 'customer.name': { $regex: search, $options: "i" } }, // Changed to customer.name
                { 'customer.mobileNumber': { $regex: search, $options: "i" } } //Added customer.
            ],

        })
            .populate({ path: "bookBy", select: "name" })
            .populate({ path: "customer", select: "name address mobileNumber" }) // Added address and mobileNumber
            .populate({ path: "repairing.fault", select: "fault" })
            .populate({ path: "deliverBy", select: "name" })
            .populate({ path: "repairing.repairUnder", select: "name" })
            .populate({ path: "repairing.repairBy", select: "name" })
        ...
        if (!repairs || repairs.length === 0) { // Changed the condition
            return res.status(404).json(new ApiResponse(404, [], "No Repairs found")); // return empty array
        }
